...
```{r make-cora, echo=FALSE, fig.cap="One Item x Item Correlation Matrix as a Heatmap", fig.height=10, fig.width=10}
...
```{r low-level-pca, eval = FALSE, include = FALSE}
t(komki$qcat$ass$Azra[c("comma", "i-we"),])
t(surprise$Azra[c("comma", "i-we"),])


testpca <- prcomp(x = cora[,,"Azra"], retx = TRUE, center = TRUE, scale. = TRUE)
testpca$sdev^2
library(GPArotation)
varimax <- unclass(Varimax(L = testpca$rotation[,1:6])$loadings)

ggplot(data = as.data.frame(varimax), mapping = aes(x = PC1, y = PC2, label = rownames(testpca$rotation)))+ geom_text() #  + xlim(-1,1) + ylim(-1,1)
sum(testpca$sdev^2)

summary(cora)
which(cora == -1, arr.ind = TRUE)
qplot(x=item_x, y=item_y, data=melt(cora[,,"Azra"], varnames = c("item_x", "item_y")), fill=value, geom="tile")

head(melt(cora[,,"Azra"]))
q.corrplot(corr.matrix = cora[,,"Azra"])
